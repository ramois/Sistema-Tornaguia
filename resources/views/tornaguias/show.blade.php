<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-900 leading-tight">
            {{ __('Detalle de Tornaguia') }} #{{ $tornaguia->id }}
        </h2>
    </x-slot>
            ...
            <div class="rounded-2xl bg-white/80 backdrop-blur border border-slate-200 shadow-sm">
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                            <div class="text-sm text-slate-500">COOPERATIVA MINERA "PACOCOLLO" R.L.</div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('tornaguias.print', $tornaguia->id) }}" class="px-4 py-2 bg-slate-900 text-white rounded-md hover:bg-slate-800">Imprimir</a>
                            ...
                            <a href="{{ route('tornaguias.index') }}" class="px-4 py-2 bg-slate-200 text-slate-900 rounded-md hover:bg-slate-300">Volver</a>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="rounded-xl border border-slate-200 p-4">
                                <div class="text-sm font-semibold text-slate-900 mb-2">Acciones</div>
                                <form method="POST" action="{{ route('tornaguias.destroy', $tornaguia->id) }}" onsubmit="return confirm('Eliminar tornaguia?')">
                                    @csrf
                                    @method('DELETE')
                                    <button class="w-full px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700" type="submit">Eliminar</button>
                                </form>
                            </div>
                        </div>
                    </div>
